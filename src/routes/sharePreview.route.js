...
router.get('/jobs/:id', async (req, res) => {
  try {
      ...
      return res.status(404).send(`
        ...
        <html lang="vi">
          ...
          <body>
            ...
            <script>
              setTimeout(() => {
                window.location.href = '${process.env.CANDIDATE_FE_URL || 'http://localhost:5173'}';
              }, 3000);
            </script>
          </body>
        </html>
    ...
    if (companyLogo.startsWith('http')) {
      ...
      absoluteLogoUrl = `${process.env.CANDIDATE_FE_URL || 'http://localhost:5173'}${companyLogo}`;
    } else {
    ...
    console.log('Company Logo Debug:', {
      ...
      ogImageUrl,
      isLogoLargeEnough,
      frontendUrl: process.env.CANDIDATE_FE_URL
    });

    // Actual job URL on frontend
    const actualJobUrl = `${process.env.CANDIDATE_FE_URL || 'http://localhost:5173'}/jobs/${id}`;
